...
<head>
  ...
  <title>${title} | ARM AUTO EXPRESS</title>
  <style>
    :root { color-scheme: light; font-family: Arial, sans-serif; color: #17212b; background: #f5f7fb; }
    body { margin: 0; padding: 40px 20px; }
    main { max-width: 820px; margin: 0 auto; background: #fff; border: 1px solid #d9e1ec; border-radius: 10px; padding: 32px; }
...
<body>
  <main>
    <h1>${title}</h1>
    ${body}
    <p class="muted">Последнее обновление: 02.05.2026</p>
  </main>
</body>
</html>`;
}
...
app.get('/privacy', (req, res) => {
  res.type('html').send(renderLegalPage('Политика конфиденциальности', `
    <p>ARM AUTO EXPRESS использует WhatsApp и Telegram ботов для предоставления клиентам информации о статусах контейнеров, документах и связанных уведомлениях.</p>
    <h2>Какие данные мы обрабатываем</h2>
    <ul>
      <li>номер телефона или идентификатор чата;</li>
      <li>имя профиля, если оно передано мессенджером;</li>
      <li>сообщения, отправленные пользователем боту;</li>
      <li>запрошенные номера контейнеров и технические статусы доставки сообщений.</li>
    </ul>
    <h2>Для чего используются данные</h2>
    <p>Данные используются только для обработки запросов пользователя, отправки статусов контейнеров, документов и служебных уведомлений.</p>
    <h2>Передача данных</h2>
    <p>Мы не продаем персональные данные. Данные могут обрабатываться сервисами Meta WhatsApp Cloud API, Telegram, Google Sheets, Google Drive и хостинг-провайдером, необходимыми для работы сервиса.</p>
    <h2>Контакты</h2>
    <p>По вопросам конфиденциальности и удаления данных напишите на email, указанный в настройках приложения Meta.</p>
  `));
});

app.get('/terms', (req, res) => {
  res.type('html').send(renderLegalPage('Пользовательское соглашение', `
    <p>Используя бота ARM AUTO EXPRESS, пользователь соглашается получать ответы на свои запросы о контейнерах, документах и статусах доставки.</p>
    <h2>Назначение сервиса</h2>
    <p>Сервис предоставляет справочную информацию по контейнерам и документам на основании внутренних источников данных компании.</p>
    <h2>Ограничения</h2>
    <p>Информация может обновляться с задержкой и не является юридическим документом. Для официального подтверждения данных обращайтесь к представителю компании.</p>
    <h2>Доступ</h2>
    <p>Доступ к документам может предоставляться только авторизованным номерам телефонов или пользователям.</p>
  `));
});

app.get('/data-deletion', (req, res) => {
  res.type('html').send(renderLegalPage('Удаление данных пользователей', `
    <p>Пользователь может запросить удаление своих данных, связанных с использованием бота ARM AUTO EXPRESS.</p>
    <h2>Как запросить удаление</h2>
    <p>Отправьте запрос на email, указанный в настройках приложения Meta, или напишите администратору сервиса. В запросе укажите номер телефона или идентификатор чата, который использовался для общения с ботом.</p>
    <h2>Какие данные удаляются</h2>
    <ul>
      <li>история сообщений с ботом;</li>
      <li>имя профиля, если оно было сохранено;</li>
      <li>доступы к документам и подписки на обновления, связанные с указанным номером или чатом.</li>
    </ul>
    <h2>Срок обработки</h2>
    <p>Запрос на удаление данных обрабатывается в разумный срок после подтверждения принадлежности номера или чата.</p>
  `));
});
